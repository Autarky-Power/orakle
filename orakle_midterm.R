...
a<- orakle.mid_term_lm(midterm_all$midterm)
## Ridge----
MLmetrics::RMSE(a$midterm_model_fit,a$seasonal_avg_hourly_demand)
...
ggplot()+ geom_line(aes(1:length(a$seasonal_avg_hourly_demand),a$seasonal_avg_hourly_demand,color="actual"))+
  geom_line(aes(1:length(a$seasonal_avg_hourly_demand),a$midterm_model_fit,color="fitted"))+
